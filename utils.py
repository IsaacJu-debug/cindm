...
class Printer(object):
    ...
    def print(
        ...
        if is_datetime:
            str_time, time_second = get_time(
                return_numerical_time=True, precision=precision
            )
            string += str_time
        ...
        if avg_window != -1 and len(self.limit_list) >= 2:
            string += (
                "   \t{0:.{3}f}s from last print, {1}-step avg: {2:.{3}f}s".format(
                    ...
                    (
                        self.limit_list[-1]
                        - self.limit_list[-min(avg_window + 1, len(self.limit_list))]
                    )
                    / avg_window,
                    self.n_digits,
                )
...
def compute_orthonormal(boundary):  ## set the ortho-normal values based on two points
    ...
    length = (boundary - rolled_boundary).norm(
        dim=1
...
def linear_transform(pressure, center):
    ...
    x = torch.minimum(
        torch.maximum(center[:, 0], p_5),
        torch.tensor([n - 1.5], device=device).repeat(num_bound),
    )
    # print("x: ", x)
    x_inds = torch.minimum(
        x.type(torch.int32), torch.tensor([n - 2], device=device).repeat(num_bound)
    )
    fs = x - x_inds

    y = torch.minimum(
        torch.maximum(center[:, 1], p_5),
        torch.tensor([m - 1.5], device=device).repeat(num_bound),
    )
    y_inds = torch.minimum(
        y.type(torch.int32), torch.tensor([m - 2], device=device).repeat(num_bound)
    )
    ft = y - y_inds
...
def get_item_1d(data, target):
    ...
    x = (
        x.reshape(-1, n_bodies, n_steps, feature_size) / 200.0
    )  # [B, n_bodies, n_steps, feature_size]
...
def get_item_1d_for_solver(data, target):
    ...
    n_steps, feature_size = x.shape[1:]
    x = x.reshape(
        -1, n_bodies, n_steps, feature_size
    )  # [B, n_bodies, n_steps, feature_size]
    x = x.permute(0, 2, 1, 3)  # [B, n_steps, n_bodies, feature_size]
    x = torch.flatten(x, -2, -1)  # [B, n_steps, n_bodies*feature_size]
    return x
...
def filter_isolated_points(hard_boundary):
    ...
    while len(iso_points) != 0:
        index_tensor = torch.tensor(
            iso_points, dtype=torch.long, device=hard_boundary.device
        ).t()
...
def find_pareto_frontier(objective_values):
    ...
    np_pareto_frontier2 = np_pareto_frontier2[
        np.flip(np_pareto_frontier2[:, 0].argsort())
    ]
    ...
    np_pareto_frontier3 = np_pareto_frontier3[
        np.flip(np_pareto_frontier3[:, 0].argsort())
    ]
    ...
    org_pareto_frontier = np.concatenate(
        [
            np_pareto_frontier1,
            np_pareto_frontier2,
            np_pareto_frontier3,
    ...
    for i in range(len(pareto_frontier)):
        dir_x = (
            pareto_frontier[i % len(pareto_frontier)][0]
            - pareto_frontier[(i + 1) % len(pareto_frontier)][0]
        )
        dir_y = (
            pareto_frontier[i % len(pareto_frontier)][1]
            - pareto_frontier[(i + 1) % len(pareto_frontier)][1]
        )
        if dir_y == 0:
            if dir_x > 1:
                for k in range(1, int(dir_x)):
                    put_values.append(
                        [
                            pareto_frontier[i % len(pareto_frontier)][0] - k,
                            pareto_frontier[i % len(pareto_frontier)][1],
                        ]
                    )
            ...
            if dir_y > 1:
                for k in range(1, int(dir_y)):
                    put_values.append(
                        [
                            pareto_frontier[i % len(pareto_frontier)][0],
                            pareto_frontier[i % len(pareto_frontier)][1] - k,
                        ]
                    )
                    put_indices.append(i + 1)
            elif dir_y < -1:
    ...
    if len(put_indices) > 0:
        final_pareto = np.insert(
            np.array(pareto_frontier), put_indices, put_values, axis=0
        )
        return final_pareto
...
def find_cluster_boundary(np_clustered_grid, is_boundary_plot=False):
    ...
    for i in range(num_cluster):
        ...
        # visited = [[False] * len(grid[0]) for _ in range(len(grid))]
        boundary = bfs_boundary_detection_without_intersection(
            starting_point[0], starting_point[1], single_clustered_grid
        )
        # print(boundary)
...
def compute_binary_pressForce(pressure, np_clustered_grid, is_boundary_plot=False):
    ...
    # pdb.set_trace()
    final_boundaries = find_cluster_boundary(
        np_clustered_grid, is_boundary_plot=is_boundary_plot
    )
    ...
    for i in range(len(final_boundaries)):
        ...
        # Compute force
        pressures[i + 1] = compute_pressForce(
            pressure, torch.tensor(boundary.copy(), device=pressure.device) + 0.5
        )
    return pressures
...
def find_cells_inside_curve(polygon, grid_shape):
    ...
    for y in range(height):
        ...
        for i in range(0, len(intersections), 2):
            x_start, x_end = (
                int(np.ceil(intersections[i])),
                int(np.floor(intersections[i + 1])),
            )
...
def update_static_masks(torch_con_boundary):
    ...
    x_idx = (
        torch.cat([(sum_indices == ind_u).nonzero()[0] for ind_u in ind_unique])
    ).sort()[0]
    # print(sum_indices[x_idx])
    # print(x_idx)
    repeat_sum_indices = torch.tile(sum_indices, (ind_unique.shape[0], 1))
    repeat_ind_unique = torch.tile(
        sum_indices[x_idx].reshape(ind_unique.shape[0], 1), (1, sum_indices.shape[0])
    )
    ...
    for bdpt in range(sum_indices[x_idx].shape[0]):
        ...
        if pointy_hash == sum_indices[x_idx][bdpt]:
            base_pt = (
                torch.stack([x_inds[org_mask[bdpt]][0], y_inds[org_mask[bdpt]][0]])
                + 0.5
            )
            ...
            base_pts.append(base_pt)
            relvec = torch_con_boundary[org_mask[bdpt]] - base_pt.repeat(
                torch_con_boundary[org_mask[bdpt]].shape[0], 1
            )
            ...
            bd_points = torch.cat(
                [x_pts[0:1], x_pts[1:-1].repeat(1, 2).reshape(-1, 2), x_pts[-1:]], dim=0
            )
            dire_vec = bd_points[0::2] - bd_points[1::2]
            const = (
                bd_points[0::2, 1]
                - bd_points[0::2, 0] * dire_vec[:, 1] / dire_vec[:, 0]
            )
            ...
            # import pdb
            # pdb.set_trace()
            ax, by, con = find_orthogonal_line(
                slope,
                -torch.ones((const.shape[0],), device=torch_con_boundary.device),
                const,
                base_points[:, 0],
                base_points[:, 1],
            )
    ...
    offset_grid_bound[x_inds, y_inds] = torch.tensor(
        [1, 1], dtype=torch.float32, device=torch_con_boundary.device
    )
    ...
    offset_grid = find_cells_inside_curve(
        torch.stack((x_inds, y_inds), -1).detach().cpu().tolist(), (62, 62)
    )
    # fig, ax = plt.subplots(figsize=(4,4), ncols=1)
    ...
    offset_union = offset_grid_bound + torch.tensor(
        offset_grid, device=torch_con_boundary.device
    )
    offset_union[(offset_union.sum(-1) > 2), :] = torch.tensor(
        [1, 1], dtype=torch.float32, device=torch_con_boundary.device
    )
    ...
    union = grid_bound.transpose(1, 0) + torch.tensor(
        inner_solid_mask, device=torch_con_boundary.device
    )
    union[union == 2] = 1
...
def update_bdfeature(reconstructed_boundary):
    ...
    multi_static_feat = torch.stack([static_feature for _ in range(4)], -2).reshape(
        -1, 4, 3
    )
...
def update_data(reconstructed_bound, optimdata, original_data, constant_var, opt_var):
    ...
    batch.edge_index = {
        ("n0", "0", "n0"): optimdata.edge_index[("n0", "0", "n0")].detach().clone()
    }
    batch.edge_label_index = {
        ("n0", "0", "n0"): optimdata.edge_label_index[("n0", "0", "n0")]
        .detach()
        .clone()
    }
    ...
    batch.node_feature = {
        "n0": torch.cat(
            (mul_static_feat, original_data.node_feature["n0"][..., 3:].detach()), -1
        )
    }
    ...
    batch.param = {
        "n0": torch.cat((constant_var, opt_var), 0).transpose(1, 0).flatten()[None, :]
    }
...
class CustomSampler(Sampler):
    def __init__(
        self, data, batch_size, noncollision_hold_probability, distance_threshold
    ):
        self.data = data
...
class CustomLoss(nn.Module):
    ...
    def forward(self, predicted, target):
        ...
        predicted_reshape = predicted.reshape(
            predicted.shape[0], predicted.shape[1], int(predicted.shape[2] / 4), 4
        )
        target_reshape = target.reshape(
            target.shape[0], target.shape[1], int(target.shape[2] / 4), 4
        )
        loss2 = torch.abs(
            (predicted_reshape[:, :, :, 2]) ** 2
            + (predicted_reshape[:, :, :, 3]) ** 2
            - ((target_reshape[:, :, :, 2]) ** 2 + (target_reshape[:, :, :, 3]) ** 2)
        ).reshape(loss.shape[0], loss.shape[1], 2, 1)
        ...
        return torch.cat(
            [
                loss.reshape(loss.shape[0], loss.shape[1], 2, int(loss.shape[2] / 2)),
                loss2,
            ],
            dim=3,
        )
...
def visulization(
    filename, cond, pred, n_bodies, conditioned_steps, rollout_steps, num_features
):
    ...
    for i in range(1):
        ...
        if conditioned_steps != 0:
            cond_reshape = cond.reshape(
                cond.shape[0], conditioned_steps, n_bodies, num_features
            ).to("cpu")
        pred_reshape = pred.reshape(
            cond.shape[0], rollout_steps, n_bodies, num_features
        ).to("cpu")
        ...
        plt.title("reverse", fontsize=fontsize)
...
def run_simulation(
    ...
    if filename != None:
        videofile = (
            filename
            + f"/trajectory_balls_{n_bodies}_simu_{n_simulation+1}_steps_{n_steps}.gif"
        )
...
def simulation(
    features, n_steps, filename=None, width=200, height=200, radius=20, mass=1
):
    """
    ...
    if filename != None:
        filename2 = (
            filename
            + f"/trajectory_balls_{n_bodies}_simu_{batch_size}_steps_{n_steps}.npy"
        )
        whether_saved = np.save(
            filename2, all_position_data_save
        )  ##[n_simulations, n_steps, n_bodies, 4 ] 4 inculde (x,y,vx,vy)：
        if whether_saved:
...
def eval_simu(cond_design, design_fn, n_bodies, rollout_steps, time_interval=4):
    ...
    pred_simu = pred_simu.to(cond_design.device) / 200.0
...
def get_random_walk_noise(pos_seq, noise_std):
    ...
    acc_noise = np.random.normal(
        0, noise_std / n_step_vel**0.5, size=noise_shape
    ).astype(np.float32)
    vel_noise = np.cumsum(acc_noise, axis=1)
    pos_noise = np.cumsum(vel_noise, axis=1)
    if pos_seq.shape[1] != 1:
        pos_noise = np.concatenate([np.zeros_like(pos_noise[:, :1]), pos_noise], axis=1)
...
def to_np_array(*arrays, **kwargs):
    ...
    for array in arrays:
        ...
        if (
            ...
            or isinstance(array, torch.FloatTensor)

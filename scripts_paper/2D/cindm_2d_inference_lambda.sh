...
inference_2d_guidance_path="./saved/inference_2d_guidance_lambda/"
log_path="./logs_guidance_lambda"

# Define arrays for lambda values and num_boundaries
lambda_options=(0.0 1.0 10.0 100.0)
num_boundaries_options=(1 2)
...
for num_boundaries in "${num_boundaries_options[@]}"; do
    for lambda_force in "${lambda_options[@]}"; do
        for lambda_physics in "${lambda_options[@]}"; do

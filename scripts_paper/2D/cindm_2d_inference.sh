...
base_cmd="python inference/inverse_design_2d.py \
    ...
    --save_residuals True --inference_result_path ./saved/inference_2d_new/"
    ...
    for num_boundaries in "${num_boundaries_options[@]}"; do
        if [ "$use_physics" = "True" ]; then
            for lambda_physics in "${physics_lambda_options[@]}"; do
                ...
                log_file="logs/run_physics${use_physics}_boundaries${num_boundaries}_lambda${lambda_physics}.log"
            ...
            # Create a unique log file name
            log_file="logs/run_physics${use_physics}_boundaries${num_boundaries}.log"

...
def asynchronous_clamp(x):
    if x.dim() == 5:
        ...
        x[:, :, :-3].clamp_(
            -1.0, 1.0
        )  # clamp states (pressure and velocity of frames time steps) of number_boundary boundaries
        x[:, :, -3].clamp_(
            0.0, 1.0
        )  # clamp boundary masks of number_boundary boundaries
        x[:, :, -2:].clamp_(
            -0.5, 0.5
        )  # clamp boundary offsets of number_boundary boundaries
    elif x.dim() == 4:
        # (batch_size * number_boundary) x (3*frames + 3) x 64 x 64
        x[:, :-3].clamp_(
            -1.0, 1.0
        )  # clamp states (pressure and velocity of frames time steps) of number_boundary boundaries
        x[:, -3].clamp_(0.0, 1.0)  # clamp boundary masks of number_boundary boundaries
        x[:, -2:].clamp_(
            -0.5, 0.5
        )  # clamp boundary offsets of number_boundary boundaries
...
class GaussianDiffusion(nn.Module):
    def __init__(
        ...
        *,
        ...
        assert (
            objective in {"pred_noise", "pred_x0", "pred_v", "pred_optimal_design"}
    ...
    def share_states_over_boundaries(
        self, shape, x, use_average_share=True
    ):  # TODO: use_average=False
        ...
        states = x[
            ...
            shape[0], shape[1], self.frames, 3, shape[3], shape[4]
        )  # states: batch_size x num_boundaries x frames x 3 x 64 x 64
        if use_average_share:
            states = states.mean(
                dim=1, keepdim=True
            )  # output states: batch_size x 1 x frames x 3 x 64 x 64
        else:
        ...
        x[:, :-3] = states.reshape(
            shape[0] * shape[1], shape[2] - 3, shape[3], shape[4]
        )  # x[:,:-3]: (batch_size * num_boundaries) x (frames * 3) x 64 x 64
    ...
    def model_predictions(
        ...
        if self.objective == "pred_noise":
            ...
            if share_noise:
                pred_noise = self.share_states_over_boundaries(
                    shape, pred_noise, use_average_share=self.use_average_share
                )
    ...
    def p_mean_variance(self, shape, x, t, x_self_cond=None, clip_denoised=True):
        preds = self.model_predictions(
            shape, x, t, x_self_cond, share_noise=self.share_noise
        )
        x_start = preds.pred_x_start
        if clip_denoised:
            x_start.clamp_(-1.0, 1.0)
        if not self.share_noise:
            x_start = self.share_states_over_boundaries(
                shape, x_start, use_average_share=self.use_average_share
            )
        ...
        if not self.share_noise:
            model_mean = self.share_states_over_boundaries(
                shape, model_mean, use_average_share=self.use_average_share
            )
    ...
    def p_sample(
        ...
        if "recurrence" not in design_guidance:
            ...
            noise = (
                self.sample_noise(shape, device).view(-1, shape[2], shape[3], shape[4])
                if t > 0
                else 0
            )
            ...
            for recurrence_n in range(recurrence_times):
                        ...
                        if design_guidance.startswith("universal-forward-recurrence"):
                            ...
                            with torch.enable_grad():
                                ...
                                for kk in range(self.backward_steps):
                                    ...
                                    x_clone = x_clone.clone().detach().requires_grad_()
                            delta_x0 = (
                                x_clone.clone().detach() - x_start.clone().detach()
                            )
                            grad_design_final = (
                                grad_design_final
                                - extract(
                ...
                noise_prime = self.sample_noise(shape, device).view(
                    -1, shape[2], shape[3], shape[4]
                )
                x = (
                    extract(
                        torch.sqrt(self.alphas_cumprod / self.alphas_cumprod_prev),
                        batched_times,
                        x.shape,
                    )
                    * pred_img
    ...
    def p_sample_loop(
        ...
        for t in tqdm(
            ...
            )  # output img: [batch_size * num_boundaries, 15, 64, 64]
            ...
            img = img.view(
                shape[0], shape[1], shape[2], shape[3], shape[4]
            )  # output img: [batch_size, num_boundaries, 21, 64, 64]
            x_start = x_start.view(
                shape[0], shape[1], shape[2], shape[3], shape[4]
            )  # output x_start: [batch_size, num_boundaries, 21, 64, 64]
            final_img = img
    ...
    def ddim_sample(self, shape, cond, return_all_timesteps=False):
        ...
        img = torch.randn(
            (shape[0], shape[1] - cond.size(1), shape[2], shape[3]), device=device
        )
        # imgs = [img]
    ...
    def sample(
        ...
        return sample_fn(
            (batch_size, num_boundaries, channels, image_size, image_size),
            design_fn,
            design_guidance,
    ...
    def __init__(
        ...
        *,
        ...
        if dataset == "chaotic_ellipse":
            self.ds = Ellipse(
                dataset="chaotic_ellipse",
                input_steps=cond_frames,
                output_steps=pred_frames,
                time_interval=ts,
                is_y_diff=False,
                is_train=is_train,
            ...
            while self.step < self.train_num_steps:
                ...
                for _ in range(self.gradient_accumulate_every):
                    ...
                    if self.dataset == "chaotic_ellipse":
                        data = data["x"].reshape(
                            -1, 126, 126, self.cond_frames, 2
                        )  # / 100.

                        data_pad = torch.zeros(data.shape[0], 8, 128, 128).to(
                            data.device
                        )
                        ...
                        if (
                            ...
                            or torch.isnan(bd_mask).any()
                            or torch.isnan(bd_offset).any()
                        ):
                        ...
                        data_pad = torch.zeros(
                            data.shape[0], 3 + self.pred_frames * 3, 64, 64
                        ).to(data.device)
                        data = torch.flatten(data, -2, -1).permute(0, 3, 1, 2)
                        ...
                        cond_pad = torch.zeros(
                            cond.shape[0], 3 * self.cond_frames, 64, 64
                        ).to(data.device)
